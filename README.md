...
I used React and Redux to build my app as I found these technologies very interesting and challenging during the course, and because I felt it fitted well with the project brief, especially for the rendering of the tournament structure and the UI elements.

## Workflow

I decided to use Git for version control of the project, specifically the Gitflow method (primarily working of development branch on feature branches). This was great to reinforce the workflow learnt on the course, and helped to give clarity and focus to my approach to the task.

## Key Project Notes

I identified 2 key areas of development:
  - Understanding the Redux state structure
...
I decided to use a 2 page structure for the app:
...
I wanted to make by application to be flexible to handle any number of player names provided by the user. In order to do this byes had to be considered. 

Therefore a key concept in the early planning stages was understanding what links there are between the number of players in a tournament, the number of players at each level in the draw, and how many rounds would be required to see the tournament through to an eventual winner. I considered this important to address in the early stages as this could also have important implications for the shape of the redux state.
...
And link between initial players and players at each level of the draw:

  - Players-in-next-round = Math.ceil(players-in-previous-round / 2)

#### Revised Bye Handling Method

I altered my bye handling process during development so that byes could only occur in the first round only. This was a straightforward process due to the simple redux state structure used for the application (next section).

This new method was based on the following:
...
9 teams = 16 - 9 (7 byes)
28 teams = 32 - 28 (4 byes)

This simplified the logic as the second round onwards always has a power of 2 number of players, down to 2 finalists.

### Redux State

Having decided on a bye handling strategy I had a better idea of what the shape my redux state should look like to achieve my goals.
...
When the user clicks to the button to generate the tournament, the redux state changes to include a "rounds" property. This is an array which itself holds nested arrays of names (in a randomised order) present in each round of the draw (number of nested arrays relates to the number of rounds required for the number of input names provided).
...
This nested array structure of the rounds property meant I could effectively iterate through the names contained within them and render them to the page in either Match or Bye components.
...
The first step to coding my app was focused on the player input (home) page functionality and display. As well as new player input, features include player deletion and name editing capability.
...
This was handled in my application by, for each round, accepting the corresponding round array (holding players in that round) from redux state as a prop, and formatting the "flat" array into a nested array structure of either 1 or 2 names. This meant I could render either Match or Bye components depending on the length each item.
...
I used the CSS pre-processor SASS for styling the application, which simplified the process. Splitting styles into seperate files helped with consistancy.

The app has been built to be mobile responsive.

## Improvements

I was disappointed not to implement the ability to track match winners through to a tournament winner, however I am pleased that the app is flexible enough to allow for any number of player inputs. Maxium player limit of 32 was only added to allow for more manageable responsive styling, however it could potentially be scaled up to accomodate more.
